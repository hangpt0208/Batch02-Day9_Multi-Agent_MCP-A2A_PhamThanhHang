...
class TestTask2(unittest.TestCase):
    ...
    def test_json_files_have_metadata(self):
        ...
        if not json_files:
            self.skipTest("Không có file JSON (co the dùng format khác)")
...
class TestTask5(unittest.TestCase):
    ...
    def test_results_have_required_keys(self):
        ...
        try:
            ...
            if not results:
                self.skipTest("Không có kết quả (co the chua index)")
            for r in results:
...
class TestTask10(unittest.TestCase):
    ...
    def test_generate_returns_dict_with_answer(self):
        ...
        try:
            ...
            self.skipTest(f"Generation error (co the thieu API key): {e}")
